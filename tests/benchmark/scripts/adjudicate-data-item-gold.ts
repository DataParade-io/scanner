#!/usr/bin/env node
/**
 * Deterministic data-item adjudication pass (KDATAP-25b2f4).
 *
 *   pnpm exec ts-node tests/benchmark/scripts/adjudicate-data-item-gold.ts
 *   pnpm exec ts-node tests/benchmark/scripts/adjudicate-data-item-gold.ts --write-ledger
 *   pnpm exec ts-node tests/benchmark/scripts/adjudicate-data-item-gold.ts --apply
 */
import fs from "fs";
...
import {
  ...
  DATA_ITEM_ADJUDICATION_TASK,
  type AdjudicationLedger,
...
const LEDGER_SHORT_PATH = path.join(
  REPO_ROOT,
  "annotations/KDATAP-25b2f4/adjudication-ledger.json",
);
...
function writeLedger(ledger: AdjudicationLedger): void {
...
function loadLedger(): AdjudicationLedger {
  if (!fs.existsSync(LEDGER_SHORT_PATH)) {
    throw new Error(`Missing adjudication ledger at ${LEDGER_SHORT_PATH}`);
  }
  return JSON.parse(fs.readFileSync(LEDGER_SHORT_PATH, "utf8")) as AdjudicationLedger;
}
...
function applyLedgerToYaml(ledger: AdjudicationLedger, benchmarkRoot: string): void {
...
function printSummary(ledger: AdjudicationLedger): void {
  console.log(`Adjudication task: ${DATA_ITEM_ADJUDICATION_TASK}`);
...
function main(): void {
  ...
  if (values.apply) {
    ...
    assertAcceptCeiling(ledger);
    applyLedgerToYaml(ledger, benchmarkRoot);
    return;
  }

  const rows = listAllDataItemAnnotations(benchmarkRoot);
  const ledger = buildAdjudicationLedger(rows, benchmarkRoot);
  assertAcceptCeiling(ledger);
  printSummary(ledger);
