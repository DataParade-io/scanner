...
describe("eval/layers/data-items", () => {
  const fixtures = [
    ...new Set(dataItemEvalCases.map((caseRecord) => caseRecord.fixture)),
  ];
  ...
  it("meets data-item layer ground-truth expectations", async () => {
    ...
    const failingNegatives = report.caseResults.filter((result) => {
      const caseRecord = dataItemEvalCases.find(
        (entry) => entry.id === result.caseId,
      )!;
      return (
        caseRecord.expected.status === "negative" &&
        !result.unread &&
        !result.negativeClean
      );
    });
    ...
    expect(report.scores.recall).toBeCloseTo(4 / 7, 5);
    expect(report.scores.precision).toBeCloseTo(4 / 5, 5);
    expect(report.scores.denominators.exhaustiveScopedFindings).toBeGreaterThan(
      0,
    );
    ...
    const jvmScan = scanResults.find(
      (result) => result.fixture === "jvm-manifests-basic",
    );
