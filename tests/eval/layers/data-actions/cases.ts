...
const dataActionEvalCaseList: EvalCase[] = [
  ...
  },
  {
    ...
    layer: "data-actions",
    subject: {
      key: "asset:main (aws_db_instance)",
      name: "Main (aws_db_instance)",
    },
    evidence: { file_path: "main.tf", start_line: 5, end_line: 10 },
    expected: { status: "positive", labels: ["store"] },
    rationale:
      "aws_db_instance.main is managed PostgreSQL storage (DA-4 store).",
  },
  {
    id: "tf-s3-store",
    fixture: "terraform-basic",
  ...
  },
  {
    ...
    fixture: "ruby-basic",
    layer: "data-actions",
    subject: { key: "asset:database", name: "Ruby Basic" },
  ...
  // --- disclose (≥3 from existing fixtures) ---
  {
    ...
    rationale:
      "Outbound fetch to api.stripe.com discloses data to a third party.",
  },
  {
    ...
    expected: { status: "positive", labels: ["disclose"] },
    rationale:
      "requests.get to api.openai.com is outbound disclosure to OpenAI.",
  },
  {
  ...
  // --- collect (data-actions-basic) ---
  {
    ...
    rationale:
      "POST /signup captures email and name from the data subject (PRD collect).",
  },
  {
    id: "dab-geo-collect",
    fixture: "data-actions-basic",
    ...
    expected: { status: "positive", labels: ["collect"], documentedGap: true },
    rationale:
      "navigator.geolocation.getCurrentPosition first-captures subject location.",
  },
  {
    ...
    expected: { status: "positive", labels: ["collect"], documentedGap: true },
    rationale:
      "Segment analytics.load/track ingests subject events (collect/ingest).",
  },
  ...
  // --- generate ---
  {
    ...
    rationale:
      "scoreUser derives a new numeric score from subject features (PRD generate).",
  },
  {
    ...
    expected: { status: "positive", labels: ["generate"], documentedGap: true },
    rationale:
      "inferRisk creates a new risk label about the subject from email.",
  },
  {
  ...
  },
  {
    ...
    rationale:
      "anonymizeRecord strips email and keeps only an age bucket (anonymize→transform).",
  },
  {
    ...
    rationale:
      "aggregatePurchases reduces amounts to a single metric (aggregate→transform).",
  },
  ...
  // --- use ---
  {
    ...
    rationale:
      "approveOrder decides from in-memory balance/price without persisting.",
  },
  {
    id: "dab-plan-use",
    fixture: "data-actions-basic",
    ...
    expected: { status: "positive", labels: ["use"], documentedGap: true },
    rationale:
      "selectPlan consults userTier for a decision with no store write.",
  },
  {
    id: "dab-adult-use",
    fixture: "data-actions-basic",
  ...
  // --- combine ---
  {
    ...
    rationale:
      "mergeProfile joins local profile with CRM fields (merge→combine).",
  },
  {
    id: "dab-join-combine",
    fixture: "data-actions-basic",
    ...
    expected: { status: "positive", labels: ["combine"], documentedGap: true },
    rationale:
      "joinOrdersWithUsers joins users and orders across sources (join→combine).",
  },
  {
    ...
    expected: { status: "positive", labels: ["combine"], documentedGap: true },
    rationale:
      "enrichWithSegment enriches a user with traits from another source.",
  },
  ...
  },
  {
    ...
    expected: { status: "positive", labels: ["relay"], documentedGap: true },
    rationale:
      "forwardWebhook POSTs payload upstream without local store/use (forward→relay).",
  },
  {
    ...
    expected: { status: "positive", labels: ["relay"], documentedGap: true },
    rationale:
      "passthroughGateway returns method/url only — no meaningful processing (passthrough→relay).",
  },
  ...
  // --- display ---
  {
    ...
    rationale:
      "renderEmailPage sends HTML that surfaces subject email to the viewer.",
  },
  {
    ...
    expected: { status: "positive", labels: ["display"], documentedGap: true },
    rationale:
      "returnProfileJson returns email/name PII in an API response (display).",
  },
  {
    id: "dab-ssn-display",
    fixture: "data-actions-basic",
    ...
    expected: { status: "positive", labels: ["display"], documentedGap: true },
    rationale:
      "showSsnLast4 surfaces truncated SSN to an authenticated viewer.",
  },
  ...
  // --- log ---
  {
    ...
    rationale:
      "logger.info logs email on the same line (PRAM logging / PII co-occurrence).",
  },
  {
    id: "dab-ssn-log",
    fixture: "data-actions-basic",
    ...
    expected: { status: "positive", labels: ["log"], documentedGap: true },
    rationale:
      "logger.error includes ssn on the same call — shadow store via logs.",
  },
  {
    id: "dab-phone-log",
    fixture: "data-actions-basic",
  ...
  // --- delete (existing java + data-actions-basic) ---
  {
    ...
    rationale:
      "repository.delete(id) on DELETE /customers/{id} is disposal (PRD delete).",
  },
  {
    id: "dab-http-delete",
    fixture: "data-actions-basic",
    ...
    expected: { status: "positive", labels: ["delete"], documentedGap: true },
    rationale:
      "DELETE /users/:id calls userStore.erase — retention enforcement.",
  },
  {
    id: "dab-purge-delete",
    fixture: "data-actions-basic",
  ...
  },
  {
    ...
    expected: { status: "positive", labels: ["store"], documentedGap: true },
    rationale:
      "db.insertUser persists checkout customer data on the same checkout-api node.",
  },
  {
    ...
    rationale:
      "fetch to api.stripe.com/v1/charges discloses payment PII from checkout-api.",
  },
  {
    id: "dab-checkout-log",
    fixture: "data-actions-basic",
    ...
    expected: { status: "positive", labels: ["log"], documentedGap: true },
    rationale:
      "logger.info logs email on the same checkout handler (disclose+store+collect sibling verbs).",
  },
  ...
  },
  {
    ...
    expected: { status: "positive", labels: ["store"], documentedGap: true },
    rationale:
      "db.insertHashed persists the pseudonym after transform on hash-writer.",
  },
  ...
  // asset:crm-sync — combine + store
  {
    ...
    expected: { status: "positive", labels: ["combine"], documentedGap: true },
    rationale:
      "syncCrmProfile merges local + CRM records (combine) before upsert.",
  },
  {
  ...
  },
  {
    ...
    expected: { status: "positive", labels: ["log"], documentedGap: true },
    rationale:
      "edge-proxy logs authorization metadata while also relaying upstream.",
  },
  ...
  // Negatives — DA-1 actor must not carry dataActions
  {
    ...
    rationale:
      "DA-1: actor nodes must never receive dataActions (store must not fire).",
  },
  {
    ...
    rationale:
      "DA-1: actor nodes must never receive dataActions (collect must not fire).",
  },
  {
    ...
    expected: { status: "negative", labels: ["disclose"] },
    rationale:
      "DA-1: actor nodes must never receive dataActions (disclose must not fire).",
  },
  ...
  // =========================================================================
  {
    ...
    expected: { status: "positive", labels: ["collect"], documentedGap: true },
    rationale:
      "FastAPI POST /signup captures email/name from the subject (collect).",
  },
  {
    id: "dap-order-store",
    fixture: "data-actions-python",
  ...
  },
  {
    ...
    expected: { status: "positive", labels: ["disclose"], documentedGap: true },
    rationale:
      "requests.post to api.stripe.com discloses payment PII (disclose).",
  },
  {
    id: "dap-signup-log",
    fixture: "data-actions-python",
  ...
  },
  {
    ...
    expected: { status: "positive", labels: ["disclose"], documentedGap: true },
    rationale:
      "checkout handler POSTs payment PII to api.stripe.com (disclose).",
  },
  {
    id: "dap-checkout-log",
    fixture: "data-actions-python",
  ...
  },
  {
    ...
    expected: { status: "positive", labels: ["store"], documentedGap: true },
    rationale:
      "PDO INSERT INTO customers persists checkout PII on checkout-api.",
  },
  {
